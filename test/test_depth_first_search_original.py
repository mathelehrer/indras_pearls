...
class depth_first_search_original_test(unittest.TestCase):
    ...
    def test_sequence2(self):
        ...
        for i in range(100, 200):
            ...
            plt.clf()
            plt.plot(np.real(dfs.points), np.imag(dfs.points))
            fig.savefig("/home/jmartin/PycharmProjects/indras_pearls/tmp/figure2_" + str(i) + "_" + str(
                np.round(y * 10) / 10) + ".png", dpi=100)
            # plt.show()
            # Plotter.plot(*dfs.circs) # only used this for level_max<6
    ...
    def test_Apollonian(self):
        dfs = DepthFirstSearchOriginal(ApollonianGasket, eps=0.1)
        dfs.run(close_curve=True)
        plt.clf()
        plt.gca().set_aspect('equal')
        plt.plot(np.real(dfs.points), np.imag(dfs.points))
    ...
    def test_generate_Apollonian_data(self):
        eps = 0.1
        while eps > 0.00001:
            dfs = DepthFirstSearchOriginal(ApollonianGasket, eps=eps)
            dfs.run(close_curve=True)
            fp = open(r'/home/jmartin/blendermath/video_apollonian/data/apollonian_' + str(eps) + '.dat', 'w')
            first = True
            for p in dfs.points:
                if not first:
                    fp.write(",")
